...
lines imply that there are empty rows. But this is not a problem at all. R
will ignore them and we are good to go.
...
the call we want to know, whether the function has worked and if there an
...
only selected columns. Further, we used bind_rows() to add a new column

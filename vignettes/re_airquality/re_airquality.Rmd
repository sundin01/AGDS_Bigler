...
$120\: \frac{\mu g}{m^3}$ (Brönnimann 2013, 23). There are other
definitions for a threshold, but here we use this definition. However, this
unit depends on the air pressure, which decreases with altitude. The volume
...
the air quality is acceptable. But there are measurements every month that
exceed the threshold value (maximum \> 60 ppm). For July, August, and
September, there are even more values (0.75 quantiles \> 60 ppm). That
means that there are isolated days when the ozone concentration is
problematic. If we compare the standard deviation and IQR, we notice that
they are not so different. If we also take the span into account, we notice
that the core of the data has a relatively small scatter. In the range of
0.25 and 0.75, the scatter increases, especially for the months of July and
...
ozone distribution is linear or monotonic, we use the "Pearson" and
"Spearman" methods. The graphic shows us a high positive correlation
...
values in June). For that, we need a model. Here, we create a simple linear
regression model. For that, we calculated four different models.
...
lowered, which would also inhibit ozone production. Unfortunately, we could
not answer our research question. We suppose it is possible if we collect
more data. We are convinced (and our analysis seems to support the thesis)

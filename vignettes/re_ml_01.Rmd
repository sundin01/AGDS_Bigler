...
Click one the link to use the original data:

<https://raw.githubusercontent.com/geco-bern/agds/main/data/FLX_CH-Dav_FLUXNET2015_FULLSET_DD_1997-2014_1-3.csv>
...
Here we split our data in a trining set and a test set. If you load the
file, the data will split automatically (70 % training and 30 % test).
After we split the data we can calculate our model. For KNN, we use k = 8.
This choice is random. The last code chunk in this workflow will show you,
which k is the optimal one (we do not want to spoiler here. That is why you
should use k = 8 first).

```{r splitting_data_in_train_and_test}
# Load the function in the file.
# Load of this file will also create the training and the test data without a function call
...
The situation of KNN is different. KNN calculate a "local regression". It
use not only all predictors. It uses k neighbours to calculate a local
...
##Hypothesis: The lower we chose k the higher is RSQ in the training data
but the higher the variance in the test data and therefore the higher the
MAE.
...
tends to be underfitted because there are to many neighbours. We see that
in the increasing of the MAE with increaing of k.
...
test data hyperbola (green point = 25).

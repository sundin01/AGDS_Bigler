...
if (!file.exists(name.of.file)){
  ...
  }else{daily_fluxes <- read_csv("../../data/re_ml_01/daily_fluxes.csv")}
```
...
daily_fluxes <- use.good.quality(daily_fluxes)
...
eval_model(mod = mod.knn, df_train = daily_fluxes_train, df_test = daily_fluxes_test, c("Davos (knn)"), c("Davos (knn)"))
```
...
# Function call
different.k(my.sequence,daily_fluxes_train,daily_fluxes_test,c("Davos (knn)"),c("Davos (knn)"))
```
...
extract.mae(mod.knn, daily_fluxes_test)

extract.rsq(mod.knn, daily_fluxes_test)

...
conflict, we can (1) name the package name before the function call or (2)
we can set a priority. In this exercise we will set a preference in the
first line in the code chunk.
...
affect the health. (Brönnimann 2013, 23). The threshold value for
Switzerland is $120\: \frac{\mu g}{m^3}$ over eight hours (Brönnimann 2013,
23). However, this unit depends on the air pressure, which decreases with
altitude. The volume mixing ratio with the unit parts per million is more
suitable which is about 60 ppb which we use in this exercise. (Brönnimann
2013, 26).
...
NA and (2) th e gap between to values is to big. We would only falsify our
analysis.
...
ggplot(data = airquality_mutated, aes(x = factor(Month), y = Ozone)) +
...
# count and write how many values are in the boxplot (without NA!)  
  annotate("text",
           x = 1:length(table(airquality_mutated$Month)),
           y = 180,
...
or monotonic, we use the "Pearosn" and the "Spearman" method. The graphic

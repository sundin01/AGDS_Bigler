...
# add title and subtitle and label the axes  
    ggtitle("Density of the ozone concentration", 
...
```{r}
airquality_mutated |>
  ggplot(aes(x = Day, y = Ozone, group = Month, color = Month)) +
  geom_line() +
  geom_point() +
# chose a background and add a panel boarder  
  theme_bw() +
...
#use smooth for a linear regression, se for standard error  
  ...
  stat_poly_eq(formula = y~x, 
      label.x = "left", eq.with.lhs = "",
      aes(label = paste("bold(\"", 
      c("May", "Jun", "Jul", "Aug", "Sep")[stat(group)], 
      " :  \")*", "italic(hat(y))~`=`~", stat(eq.label), sep = "")), 
      label.x.npc = "left", parse = TRUE) +
  geom_hline(yintercept = 60, linetype = "dotdash", size = 0.3, color = "darkmagenta") +
#lab the axes and giv the graphic a title  
  labs(title = "Ozone vs. Time", x = "Time [Day]", y = "Ozone [ppb]") +
  theme_bw() +
  theme(panel.border = element_rect(colour = "black", fill = NA, linewidth = 1)) +
  theme( legend.background = element_rect( fill = "lightblue", 
                size = 0.5, linetype = "solid", colour = "black"))
...
#use smooth for a linear regression, se for standard error  
  geom_smooth(formula = y ~ x, method = "lm", se = FALSE) +
  stat_poly_eq(formula = y~x, label.x = "left", eq.with.lhs = "",
          aes(label = paste("bold(\"", 
          c("May", "Jun", "Jul", "Aug", "Sep")[stat(group)], 
          " :  \")*", "italic(hat(y))~`=`~", stat(eq.label), sep = "")), 
          label.x.npc = "left", parse = TRUE) +
...
#use smooth for a linear regression, se for standard error  
  geom_smooth(formula = y ~ x, method = "lm", se = FALSE) +
  stat_poly_eq(formula = y~x, label.x = "left", 
        eq.with.lhs = "",
        aes(label = paste("bold(\"",
        c("May", "Jun", "Jul", "Aug", "Sep")[stat(group)], 
        " :  \")*", "italic(hat(y))~`=`~", stat(eq.label), sep = "")), 
  ...
  geom_hline(yintercept = 60, linetype = "dotdash", 
        size = 0.3, color = "darkmagenta") +
#lab the axes and giv the graphic a title  
  labs(title = "Ozone vs. Wind", x = "Wind [m/s]", y = "Ozone [ppb]") +
  theme_bw() +
  theme(panel.border = element_rect(colour = "black", 
        fill = NA, linewidth = 1)) +
  theme( legend.background = element_rect( fill = "lightblue", 
...
#use smooth for a linear regression, se for standard error  
  ...
  geom_hline(yintercept = 60, linetype = "dotdash", 
        size = 0.3, color = "darkmagenta") +
#lab the axes and giv the graphic a title  
  labs(title = "Ozone vs. solar irradiation", x = "solar irradiation [J/m^2]", y = "Ozone [ppb]") +
  theme_bw() +
  theme(panel.border = element_rect(colour = "black", 
        fill = NA, linewidth = 1)) +
  theme( legend.background = element_rect( fill = "lightblue", 
